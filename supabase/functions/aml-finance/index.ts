...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ─── PHASE 8 — Cross-portal RBAC: Finance Portal → AML case handoff ───
    // create_case_handoff: authenticated finance-portal partner (via x-finance-session-token)
    //   requests a short-lived, read-only handoff token for the AML case bound to a client
    //   they are assigned to. Never leaks SMR / restricted fields.
    if (op === "create_case_handoff") {
      ...
      // Validate finance partner session
      ...
      if (!portalUser || !portalUser.is_active || portalUser.revoked_at) {
        return jr({ error: "Invalid finance session" }, 401);
      }
      if (!portalUser.session_expires_at || new Date(portalUser.session_expires_at) < new Date()) {
        return jr({ error: "Finance session expired" }, 401);
      }

      // Confirm partner is assigned to this client
      ...
      // Find the AML case for this client (latest)
      ...
      // Audit on the case timeline — visible to compliance only
      await appendCaseEvent(
        admin, c.id, "system",
        "Finance-portal handoff token minted (read-only, 5min)",
        { finance_user_id: portalUser.id, ip, ua },
        null, "finance-portal",
      );
    ...
    // redeem_case_handoff: exchanges a valid handoff token for a strictly whitelisted,
    //   tipping-off-safe snapshot of the AML case. NO SMR, decisions rationale,
    //   internal notes, sanctions/PEP details, or event log content is exposed.
    if (op === "redeem_case_handoff") {
      ...
      // Whitelisted discrepancy summary — kind + severity + status only, no free text
      ...
      // Evidence labels only — no metadata, no reference IDs, no URLs
      const { data: ev } = await aml.from("evidence_references")
        .select("label, reference_type, created_at")
        .eq("case_id", caseId)
        .order("created_at", { ascending: false })
        .limit(50);

      // Latest finance comparison — non-restricted commercial numbers only
      ...
      // Audit the redemption on the case
      await appendCaseEvent(
        admin, caseId, "system",
        "Finance-portal handoff snapshot viewed",
        { finance_user_id: tok.finance_user_id, ip },
        null, "finance-portal",
      );
      ...
      return jr({
        snapshot: {
          ...
          open_discrepancies: (discs ?? []).map((d: any) => ({
            kind: d.kind, severity: d.severity, status: d.status,
          })),
          evidence_summary: (ev ?? []).map((e: any) => ({
            label: e.label, reference_type: e.reference_type,
          })),
