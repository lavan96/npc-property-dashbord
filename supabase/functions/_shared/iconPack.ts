/**
 * Curated vector icon pack — Deno twin of `src/lib/reportTemplate/iconPack.ts`
 * (KEEP IN SYNC; the edge runtime cannot import from src/).
 *
 * `expandIconOverlay` turns the agent's compact `{ type:'vector', icon, color }`
 * into a fully-specified, schema-valid vector overlay: the model only ever
 * references icon NAMES, never raw path data it could hallucinate.
 */
...
/**
 * Expand `{ type:'vector', icon:'<name>', color? }` into a schema-valid vector
 * overlay (stroke glyph). Unknown names are left untouched (the renderer just
 * skips a vector with no paths — never worse than before).
 */
export function expandIconOverlay(overlay: any): any {
  if (!overlay || overlay.type !== 'vector' || typeof overlay.icon !== 'string') return overlay;
  const paths = ICON_PACK[overlay.icon];
  if (!paths) return overlay;
  const color = typeof overlay.color === 'string' && overlay.color ? overlay.color : '#111111';
  const { icon: _icon, color: _color, ...rest } = overlay;
  return {
    ...rest,
    type: 'vector',
    name: rest.name ?? `icon:${overlay.icon}`,
