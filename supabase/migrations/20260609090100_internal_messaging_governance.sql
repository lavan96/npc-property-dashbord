...
-- Safe backfill. Existing client portal records remain client-facing only unless explicitly internal.
...
WHERE visibility_scope IS NULL OR permission_status = '{}'::jsonb;

-- Existing finance messages remain Command Centre ↔ Finance private. This intentionally does not expose legacy finance/internal notes to clients.
...
-- Direct Finance ↔ Client threads need client-authored rows in the existing finance thread table.
ALTER TABLE public.finance_portal_messages
...
-- Command Centre must be notified for every Finance ↔ Client message, not only partner-authored rows.
