DROP FUNCTION IF EXISTS public.admin_set_aml_roles_for_user(uuid, text[], uuid);
DROP FUNCTION IF EXISTS public.get_aml_roles_for_users(uuid[]);
...
BEGIN
  ...
  UPDATE aml.role_assignments ra
  SET revoked_at = now(),
      updated_at = now(),
      notes = COALESCE(ra.notes, '') || CASE WHEN COALESCE(ra.notes, '') = '' THEN '' ELSE E'\n' END || 'Revoked from User Management'
  WHERE ra.user_id = _target_user_id
    AND ra.revoked_at IS NULL
    AND NOT (ra.role::text = ANY(_roles));
