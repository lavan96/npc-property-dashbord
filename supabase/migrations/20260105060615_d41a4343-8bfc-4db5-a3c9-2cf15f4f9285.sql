-- Fix the archive_report_version trigger to prevent multiple version increments
-- The trigger should only fire when report_content actually changes
...
BEGIN
  ...
  IF OLD.report_content IS NOT NULL AND OLD.report_content != '' THEN
    INSERT INTO public.report_versions (
      ...
      NEW.generated_by,
      quality,
      'Report regenerated with fresh content - version ' || next_version
    );
