...
export function CashFlowAiPanel({ children, active = true }: CashFlowAiPanelProps) {
  ...
  return (
    <section className="min-w-0 space-y-4 rounded-[1.75rem] border border-amber-300/30 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-3 shadow-2xl shadow-black/20 ring-1 ring-amber-400/15 md:p-4">
      <div className="flex min-w-0 items-center gap-3 px-1">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs leading-5 text-slate-300 [overflow-wrap:anywhere]">Generate, save, and export comparison analysis without changing report payloads.</p>
        </div>
      </div>
      {children}
    </section>
