...
export function QuantitativeReportCard({ report, generatorLabel, onView, onDownloadPDF, isFocused = false }: QuantitativeReportCardProps) {
  ...
  const dataQuality = report.analytics?.quality?.avg_confidence != null
    ...
    : report.analytics?.valid_price_count != null && report.listing_count
      ...
      : 'N/A';
  const generatedByLabel = report.generated_by
    ? generatorLabel(report.generated_by)
    : report.source_snapshot?.requested_by_username
      ? String(report.source_snapshot.requested_by_username)
      : 'System';

  return (
    <Card className={cn(
      "group relative overflow-hidden rounded-2xl border border-border/70 bg-card/90 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/10 dark:bg-background/70",
      isFocused && "border-primary/70 shadow-2xl shadow-primary/20 ring-2 ring-primary/40",
    )}>
      ...
      {isFocused && <div className="pointer-events-none absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-transparent via-primary to-transparent" aria-hidden="true" />}
