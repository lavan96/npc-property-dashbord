...
export function TotpEnrollmentCard({ disabled }: { disabled?: boolean }) {
  ...
  const [pending, setPending] = useState<{ token: string; secret: string } | null>(null);
  ...
  const begin = async () => {
    ...
    setPassword(''); setPending({ token: result.enrollmentToken, secret });
  };
  const confirm = async () => { if (!pending || !/^\d{6}$/.test(code)) return; setBusy(true); setError(null); const result = await confirmTotpEnrollment(pending.token, code); setBusy(false); if (!result.ok) return setError(result.error); setPending(null); setCode(''); };
