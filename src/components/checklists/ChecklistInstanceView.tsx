...
export function ChecklistInstanceView({ instance, onBack }: ChecklistInstanceViewProps) {
  ...
  const progressStateClass = progress === 100
    ...
    : progress === 0
      ? 'border-border/45 bg-card dark:bg-background/70 text-muted-foreground dark:text-zinc-300'
  ...
  return (
    <div className="min-h-0 space-y-6 rounded-3xl border border-brand-500/10 bg-[radial-gradient(circle_at_top_right,rgba(245,158,11,0.14),transparent_34%),linear-gradient(180deg,#09090b,#030303)] p-4 text-foreground dark:text-zinc-100 md:p-6">
      {/* Header */}
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="grid grid-cols-1 gap-2 sm:flex sm:flex-wrap sm:items-center lg:justify-end">
            <Button
              ...
              className="min-h-10 justify-center gap-1 border-border/50 bg-background dark:bg-black/30 text-muted-foreground dark:text-zinc-300 transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-300/45 hover:bg-brand-400/10 hover:text-brand-100 hover:shadow-[0_10px_24px_rgba(245,158,11,0.12)] focus-visible:ring-2 focus-visible:ring-brand-300/55 motion-reduce:transition-none"
        ...
        <div className="min-h-0 space-y-4 overflow-x-hidden">
          ...
          const sectionIconClass = sectionComplete
            ...
            : sectionStarted
              ...
              : 'border-border/40 bg-background dark:bg-black/30 text-muted-foreground dark:text-zinc-300';
              ...
              <CardContent className="space-y-1.5 px-5 pb-5">
                {section.items.map(item => (
                  ...
                  >
                    ...
                    />
                    <span className={`min-w-0 flex-1 basis-[calc(100%-2rem)] whitespace-normal break-words sm:basis-auto text-sm leading-6 ${item.is_checked ? 'text-muted-foreground dark:text-zinc-300 line-through decoration-success/70 decoration-2 underline-offset-4' : 'text-foreground dark:text-zinc-100'}`}>
                      {item.label}
                    </span>
                    {item.checked_at && (
