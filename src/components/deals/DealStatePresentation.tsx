...
const toneClass = {
  neutral: 'border-border dark:border-white/10 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.08),transparent_42%),linear-gradient(145deg,rgba(39,39,42,0.82),rgba(9,9,11,0.78))] text-foreground dark:text-zinc-100',
...
export function NoResultsState({ title = 'No results match this view', description = 'Try clearing search terms or relaxing filters to bring deals back into view.' }: { title?: string; description?: string }) {
  return <DealStatePanel tone="neutral" icon={<SearchX className="h-7 w-7 text-muted-foreground dark:text-zinc-300" />} eyebrow="No results" title={title} description={description} compact />;
}
