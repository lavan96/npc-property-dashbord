...
const toneClass = {
  neutral: 'border-white/10 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.08),transparent_42%),linear-gradient(145deg,rgba(39,39,42,0.82),rgba(9,9,11,0.78))] text-zinc-100',
  success: 'border-emerald-300/20 bg-[radial-gradient(circle_at_top,rgba(16,185,129,0.16),transparent_42%),linear-gradient(145deg,rgba(6,78,59,0.28),rgba(9,9,11,0.78))] text-emerald-50',
  warning: 'border-amber-300/25 bg-[radial-gradient(circle_at_top,rgba(245,158,11,0.18),transparent_42%),linear-gradient(145deg,rgba(120,53,15,0.24),rgba(9,9,11,0.78))] text-amber-50',
  danger: 'border-red-300/25 bg-[radial-gradient(circle_at_top,rgba(239,68,68,0.16),transparent_42%),linear-gradient(145deg,rgba(127,29,29,0.22),rgba(9,9,11,0.80))] text-red-50',
  gold: 'border-amber-200/25 bg-[radial-gradient(circle_at_top,rgba(251,191,36,0.20),transparent_42%),linear-gradient(145deg,rgba(245,158,11,0.12),rgba(9,9,11,0.78))] text-amber-50',
};
...
export function DealStatePanel({ icon, eyebrow, title, description, tone = 'neutral', action, className, compact }: StatePanelProps) {
  return (
    <div className={cn('relative overflow-hidden rounded-[1.35rem] border p-6 text-center shadow-[0_18px_55px_rgba(0,0,0,0.20),inset_0_1px_0_rgba(255,255,255,0.08)]', toneClass[tone], compact ? 'py-5' : 'py-10 sm:py-12', className)}>
      ...
      <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-white/15 bg-white/[0.07] shadow-inner">
        {icon ?? <Sparkles className="h-7 w-7 text-amber-200" />}
      </div>
...
export function DealLoadingState({ title = 'Loading deal pipeline', description = 'Preparing live deal, commission and settlement signals.' }: { title?: string; description?: string }) {
  return (
    <DealStatePanel
      tone="gold"
      icon={<Loader2 className="h-7 w-7 animate-spin text-amber-200" />}
      eyebrow="Loading"
      title={title}
...
export function DealErrorState({ title = 'Unable to load deals', message, onRetry }: { title?: string; message?: string; onRetry?: () => void }) {
  return (
    <DealStatePanel
      tone="danger"
      icon={<AlertTriangle className="h-7 w-7 text-red-200" />}
...
export function NoResultsState({ title = 'No results match this view', description = 'Try clearing search terms or relaxing filters to bring deals back into view.' }: { title?: string; description?: string }) {
  return <DealStatePanel tone="neutral" icon={<SearchX className="h-7 w-7 text-zinc-300" />} eyebrow="No results" title={title} description={description} compact />;
}
