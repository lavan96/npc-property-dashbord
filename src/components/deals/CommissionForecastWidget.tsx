...
export function CommissionForecastWidget({ deals }: Props) {
  ...
  return (
    <Card className="overflow-hidden border-emerald-500/20 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,0.16),transparent_34%),linear-gradient(135deg,rgba(2,6,23,0.96),rgba(15,23,42,0.92))] text-slate-50 shadow-[0_24px_70px_rgba(2,6,23,0.32)]">
      <CardHeader className="px-3 pb-2 pt-3 sm:px-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <CardTitle className="flex items-start gap-2 text-sm font-semibold tracking-tight">
            <span className="mt-0.5 rounded-xl border border-emerald-300/20 bg-emerald-400/10 p-1.5 shadow-inner shadow-emerald-950/40">
              <DollarSign className="h-4 w-4 text-emerald-300" />
            </span>
            <span>
              Commission Forecast
              <span className="block text-[10px] font-medium uppercase tracking-[0.18em] text-emerald-100/55">Next 6 months</span>
            </span>
          </CardTitle>
          <Badge variant="outline" className="rounded-full border-emerald-300/30 bg-emerald-300/10 px-3 py-1 text-[11px] font-semibold text-emerald-100 shadow-[0_0_26px_rgba(16,185,129,0.16)]">
      ...
      <CardContent className="px-3 pb-3 sm:px-4">
        <div className="relative rounded-2xl border border-white/10 bg-slate-950/45 p-3 shadow-inner shadow-black/30">
            ...
            {forecast.map((month, i) => {
              ...
              return (
                <div key={i} className="group relative flex flex-1 flex-col items-center gap-1.5">
                  <span className={`min-h-4 text-[9px] font-semibold tabular-nums sm:text-[10px] ${month.amount > 0 ? 'text-emerald-100/85' : 'text-slate-500'}`}>
                    ...
                    <div
                      className={`w-full rounded-t-lg border transition-all duration-300 group-hover:brightness-125 ${
                        month.amount > 0
                          ...
                          : 'border-white/10 bg-slate-700/45 shadow-inner shadow-black/30'
                      } ${isPeak ? 'ring-1 ring-lime-200/60' : ''}`}
                      style={{ height: `${height}%` }}
                      title={`${month.label}: ${fmt(month.amount)} (${month.count} deals)`}
                    />
                  </div>
                  <div className="pointer-events-none absolute bottom-12 z-10 hidden min-w-32 -translate-y-2 rounded-xl border border-emerald-200/20 bg-slate-950/95 px-3 py-2 text-center shadow-2xl shadow-black/40 group-hover:block">
                    <p className="text-[10px] font-semibold text-slate-100">{month.label}</p>
                    <p className="text-xs font-bold text-emerald-200">{fmt(month.amount)}</p>
                    <p className="text-[10px] text-slate-400">{month.count} deal{month.count === 1 ? '' : 's'}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-300 sm:text-[11px]">{month.label.split(' ')[0]}</p>
                    <p className="text-[8px] text-slate-500 sm:text-[9px]">{month.label.split(' ')[1]}</p>
                    <p className={`text-[8px] ${month.count > 0 ? 'text-emerald-200/70' : 'text-slate-600'}`}>
          ...
          {!hasForecastValue && (
            <div className="mt-3 rounded-xl border border-dashed border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-slate-400">
