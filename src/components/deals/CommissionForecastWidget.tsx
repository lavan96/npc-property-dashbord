    ...
    <Card className="overflow-hidden rounded-[1.35rem] border-amber-300/20 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,0.14),transparent_30%),radial-gradient(circle_at_top_left,rgba(245,158,11,0.12),transparent_34%),linear-gradient(145deg,rgba(39,39,42,0.94),rgba(9,9,11,0.88))] text-foreground dark:text-zinc-50 shadow-[0_24px_70px_rgba(0,0,0,0.32),inset_0_1px_0_rgba(255,255,255,0.07)]">
      ...
      <CardContent className="px-3 pb-3 sm:px-4">
        <div className="relative rounded-2xl border border-border dark:border-white/10 bg-background dark:bg-black/30 p-3 shadow-inner shadow-sm dark:shadow-black/30">
            ...
            {forecast.map((month, i) => {
              ...
              return (
                <div key={i} className="group relative flex flex-1 flex-col items-center gap-1.5">
                  <span className={`min-h-4 text-[9px] font-semibold tabular-nums sm:text-[10px] ${month.amount > 0 ? 'text-emerald-100/85' : 'text-muted-foreground dark:text-zinc-500'}`}>
                    ...
                    <div
                      className={`w-full rounded-t-lg border transition-all duration-300 group-hover:brightness-125 ${
                        month.amount > 0
                          ...
                          : 'border-border dark:border-white/10 bg-muted dark:bg-zinc-800/65 shadow-inner shadow-sm dark:shadow-black/30'
                      } ${isPeak ? 'ring-1 ring-lime-200/60' : ''}`}
                      style={{ height: `${height}%` }}
                      title={`${month.label}: ${fmt(month.amount)} (${month.count} deals)`}
                    />
                  </div>
                  <div className="pointer-events-none absolute bottom-12 z-10 hidden min-w-32 -translate-y-2 rounded-xl border border-emerald-200/20 bg-background dark:bg-zinc-950/95 px-3 py-2 text-center shadow-2xl shadow-sm dark:shadow-black/40 group-hover:block">
                    <p className="text-[10px] font-semibold text-foreground dark:text-zinc-100">{month.label}</p>
                    <p className="text-xs font-bold text-emerald-200">{fmt(month.amount)}</p>
                    <p className="text-[10px] text-muted-foreground dark:text-zinc-400">{month.count} deal{month.count === 1 ? '' : 's'}</p>
                  </div>
                  <div className="text-center">
                    <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground dark:text-zinc-300 sm:text-[11px]">{month.label.split(' ')[0]}</p>
                    <p className="text-[8px] text-muted-foreground dark:text-zinc-500 sm:text-[9px]">{month.label.split(' ')[1]}</p>
          ...
          {!hasForecastValue && (
            <div className="mt-3 rounded-xl border border-dashed border-border dark:border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-muted-foreground dark:text-zinc-400">
