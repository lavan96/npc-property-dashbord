...
import { Loader2, Send, Headphones, User as UserIcon, MessageCircle } from 'lucide-react';
    ...
    <div className="flex flex-col h-[600px] min-h-0 border border-border rounded-lg bg-card overflow-hidden">
      <div className="flex items-center justify-between border-b border-border px-4 py-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">Client Portal Messages</span>
        </div>
              ...
              return (
                <div key={m.id} className={cn('flex items-end gap-2', mine ? 'flex-row-reverse' : 'flex-row')}>
                  ...
                  <div className={cn('max-w-[78%] flex flex-col', mine ? 'items-end' : 'items-start')}>
                    ...
                    <span className="text-[10px] text-muted-foreground mt-1 px-1">{formatStamp(m.created_at)}</span>
                  </div>
                </div>
              );
            })}
      ...
      <div className="border-t border-border p-3 bg-card">
        ...
        <p className="text-[10px] text-muted-foreground mt-1.5">Press ⌘/Ctrl+Enter to send · Replies appear instantly in the client portal</p>
      </div>
    </div>
  );
}
