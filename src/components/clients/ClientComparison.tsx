...
export function ClientComparison({ clients }: ClientComparisonProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Client Selection */}
      <Card className="relative overflow-hidden rounded-3xl border-amber-400/15 bg-[radial-gradient(circle_at_top_left,rgba(245,158,11,0.13),transparent_32%),linear-gradient(145deg,rgba(24,24,27,0.95),rgba(3,7,18,0.9))] shadow-2xl shadow-[0_16px_44px_rgba(15,23,42,0.10)] dark:shadow-black/25">
        ...
        <CardHeader className="border-b border-border/60 dark:border-white/10 pb-4">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0 space-y-2">
              <div className="inline-flex items-center gap-2 rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-amber-700 dark:text-amber-100">
              ...
              <CardTitle className="flex items-center gap-3 text-xl font-bold tracking-tight text-foreground dark:text-white sm:text-2xl">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-amber-300/25 bg-amber-300/15 text-amber-700 dark:text-amber-100 shadow-lg shadow-amber-950/20">
                  <Users className="h-5 w-5" />
                </span>
                Select Clients to Compare
              </CardTitle>
              <p className="max-w-2xl text-sm leading-6 text-muted-foreground dark:text-slate-400">
        ...
        <CardContent className="space-y-5 p-4 sm:p-5">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
            <div className="relative flex-1">
              <Search className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-amber-700 dark:text-amber-100/55" />
              <Input
                placeholder="Search clients by name or email..."
                aria-label="Search clients to compare"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="h-12 rounded-2xl border-amber-300/15 bg-muted/45 dark:bg-black/20 pl-11 pr-4 text-sm text-foreground dark:text-white shadow-inner shadow-[0_14px_38px_rgba(15,23,42,0.08)] dark:shadow-black/20 placeholder:text-muted-foreground/80 dark:text-slate-500 transition-all hover:border-amber-300/30 focus-visible:border-amber-300/55 focus-visible:ring-2 focus-visible:ring-amber-300/30 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950"
              />
            </div>
            <Button
              ...
              className={`h-12 shrink-0 gap-2 rounded-2xl px-4 text-sm font-semibold transition-all hover:-translate-y-0.5 focus-visible:ring-2 focus-visible:ring-amber-300/40 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${
                showActiveOnly
                  ...
                  : 'border-border/60 dark:border-white/10 bg-background/65 dark:bg-white/[0.035] text-muted-foreground dark:text-slate-300 hover:border-amber-300/30 hover:bg-amber-300/10 hover:text-amber-700 dark:hover:text-amber-100'
              }`}
          ...
          <ScrollArea className="h-[320px] rounded-2xl border border-border/60 dark:border-white/10 bg-muted/35 dark:bg-black/15 p-3 shadow-inner shadow-[0_16px_44px_rgba(15,23,42,0.10)] dark:shadow-black/25 sm:h-[280px]">
            {filteredClients.length === 0 ? (
              <div className="flex min-h-[220px] flex-col items-center justify-center rounded-2xl border border-dashed border-amber-300/20 bg-amber-300/[0.03] px-6 py-8 text-center">
                <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-2xl border border-amber-300/20 bg-amber-300/10 text-amber-700 dark:text-amber-100">
                ...
                <p className="mt-1 max-w-sm text-xs leading-5 text-muted-foreground/80 dark:text-slate-500">
                  {showActiveOnly
                    ...
                    <div
                      ...
                      className={`group flex cursor-pointer items-center gap-3 rounded-2xl border p-3.5 transition-all duration-300 ${
                        isSelected
                          ...
                          : 'border-border/60 dark:border-white/10 bg-background/65 dark:bg-white/[0.035] opacity-90 hover:-translate-y-0.5 hover:border-amber-300/35 hover:bg-background/70 dark:bg-white/[0.06] hover:opacity-100 hover:shadow-[0_12px_28px_rgba(245,158,11,0.08)] focus-within:border-amber-300/45 focus-within:ring-2 focus-within:ring-amber-300/20'
                      }`}
                      onClick={() => toggleClient(client.id)}
                    >
                      <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl border transition-colors ${isSelected ? 'border-amber-200/60 bg-amber-300/20 text-amber-700 dark:text-amber-100' : 'border-border/60 dark:border-white/10 bg-muted/45 dark:bg-black/20 text-muted-foreground/80 dark:text-slate-500 group-hover:text-amber-700 dark:group-hover:text-amber-100'}`}>
                        <Checkbox 
                          checked={isSelected}
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          <p className={`truncate text-sm font-semibold ${isSelected ? 'text-amber-950 dark:text-amber-50' : 'text-foreground/90 dark:text-slate-200'}`}>
                        ...
                        <p className="mt-1 truncate text-xs text-muted-foreground/80 dark:text-slate-500">
                          {client.primary_email || 'No email on file'}
                        </p>
                        <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] font-semibold text-muted-foreground dark:text-slate-400">
                      ...
                      <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border text-xs font-bold transition-colors ${isSelected ? 'border-amber-200/50 bg-amber-300 text-black shadow-[0_0_18px_rgba(251,191,36,0.45)]' : 'border-border/60 dark:border-white/10 bg-muted/45 dark:bg-black/20 text-slate-600 group-hover:text-muted-foreground dark:text-slate-300'}`}>
      ...
      {selectedClients.length < 2 ? (
        <Card className="relative overflow-hidden rounded-3xl border-amber-300/15 bg-[radial-gradient(circle_at_top,rgba(245,158,11,0.16),transparent_34%),radial-gradient(circle_at_bottom_left,rgba(45,212,191,0.08),transparent_30%),linear-gradient(145deg,hsl(var(--card)),hsl(var(--muted)/0.22))] shadow-2xl shadow-[0_16px_44px_rgba(15,23,42,0.10)] dark:shadow-black/25">
          ...
          <CardContent className="px-6 py-12 text-center sm:py-16">
            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-[1.75rem] border border-amber-300/25 bg-amber-300/10 text-amber-700 dark:text-amber-100 shadow-2xl shadow-amber-950/30">
              <Scale className="h-9 w-9" />
            </div>
            <div className="mx-auto max-w-xl space-y-3">
              <div className="inline-flex items-center gap-2 rounded-full border border-border/60 dark:border-white/10 bg-background/70 dark:bg-white/[0.04] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-amber-700 dark:text-amber-100">
                <Sparkles className="h-3.5 w-3.5" />
                Decision support
              </div>
              <p className="text-2xl font-bold tracking-tight text-foreground dark:text-white">Build a comparison cohort</p>
              <p className="text-sm leading-6 text-muted-foreground dark:text-slate-400">Select at least 2 clients to unlock the side-by-side portfolio view. The output will populate here using the exact selected client data and existing comparison metrics.</p>
            </div>
            <div className="mx-auto mt-7 grid max-w-2xl gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-border/60 dark:border-white/10 bg-background/65 dark:bg-white/[0.035] px-4 py-3 text-left">
                <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground/80 dark:text-slate-500">Minimum</p>
                <p className="mt-1 text-sm font-semibold text-foreground/90 dark:text-slate-200">2 clients</p>
              </div>
              <div className="rounded-2xl border border-border/60 dark:border-white/10 bg-background/65 dark:bg-white/[0.035] px-4 py-3 text-left">
                <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-muted-foreground/80 dark:text-slate-500">Maximum</p>
              ...
              <div className="rounded-2xl border border-amber-300/20 bg-amber-300/10 px-4 py-3 text-left">
                <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-amber-700 dark:text-amber-100/60">Current</p>
                <p className="mt-1 text-sm font-semibold text-amber-950 dark:text-amber-50">{selectedClients.length}/2 ready</p>
              </div>
            </div>
            <div className="mt-6 inline-flex items-center gap-2 rounded-full border border-border/60 dark:border-white/10 bg-muted/45 dark:bg-black/20 px-4 py-2 text-xs font-semibold text-muted-foreground dark:text-slate-300">
              <UserCheck className="h-3.5 w-3.5 text-amber-700 dark:text-amber-200" />
        ...
        <div className="relative space-y-5 overflow-hidden rounded-3xl border border-border/60 dark:border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(245,158,11,0.1),transparent_28%),linear-gradient(145deg,rgba(24,24,27,0.72),rgba(3,7,18,0.58))] p-3 shadow-2xl shadow-[0_14px_38px_rgba(15,23,42,0.08)] dark:shadow-black/20 sm:p-5">
          ...
          <div className="relative flex flex-col gap-3 rounded-3xl border border-border/60 dark:border-white/10 bg-muted/45 dark:bg-black/20 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-emerald-300/20 bg-emerald-400/10 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-700 dark:text-emerald-100">
                <Scale className="h-3.5 w-3.5" />
                Comparison active
              </div>
              <h3 className="mt-3 text-xl font-bold tracking-tight text-foreground dark:text-white">Client benchmarking output</h3>
              <p className="mt-1 text-sm leading-6 text-muted-foreground dark:text-slate-400">Side-by-side metrics for {selectedClients.length} selected clients using the existing comparison calculations.</p>
            </div>
            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1.5 text-xs font-semibold text-amber-700 dark:text-amber-100">
            ...
            {selectedClients.map((client, index) => (
              <Card key={client.id} className="min-w-[210px] rounded-3xl border-amber-300/25 bg-[linear-gradient(145deg,rgba(245,158,11,0.14),rgba(255,255,255,0.035))] shadow-lg shadow-amber-950/15">
                <CardContent className="p-4">
                  ...
                  <h3 className="truncate text-base font-bold text-amber-950 dark:text-amber-50">
                    {smartCapitalize(client.primary_first_name)} {smartCapitalize(client.primary_surname)}
                  </h3>
                  <p className="mt-1 truncate text-xs text-amber-700 dark:text-amber-100/55">{client.primary_email || 'No email on file'}</p>
                  <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] font-semibold text-amber-700 dark:text-amber-100/70">
          ...
          <Card className="rounded-3xl border-border/60 dark:border-white/10 bg-[linear-gradient(145deg,hsl(var(--card))_0%,hsl(var(--muted)/0.22)_100%)] dark:bg-[linear-gradient(145deg,rgba(24,24,27,0.94),rgba(3,7,18,0.88))] shadow-xl shadow-[0_14px_38px_rgba(15,23,42,0.08)] dark:shadow-black/20">
            <CardHeader className="border-b border-border/60 dark:border-white/10 pb-4">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                ...
                <p className="text-xs font-medium text-muted-foreground/80 dark:text-slate-500">Best values are highlighted without changing comparison math</p>
              </div>
            </CardHeader>
            <CardContent className="space-y-5 p-4 sm:p-5">
              {metrics.map((metric) => {
                ...
                return (
                  <div key={metric.key} className="rounded-2xl border border-border/60 dark:border-white/10 bg-background/55 dark:bg-white/[0.025] p-3 sm:p-4">
                    <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-muted-foreground dark:text-slate-300">
                      <span className="flex h-8 w-8 items-center justify-center rounded-xl border border-border/60 dark:border-white/10 bg-muted/45 dark:bg-black/20 text-amber-700 dark:text-amber-100">
                    ...
                    >
                      {selectedClients.map((client) => {
                        ...
                        return (
                          <div 
                            key={client.id} 
                            className={`min-w-[170px] rounded-2xl border p-3 text-left ${isBest ? 'border-emerald-300/30 bg-emerald-400/10 shadow-lg shadow-emerald-950/10' : 'border-border/60 dark:border-white/10 bg-muted/45 dark:bg-black/20'}`}
                          >
                            <p className="truncate text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground/80 dark:text-slate-500">
                              {smartCapitalize(client.primary_first_name)}
                            </p>
                            <div className="mt-2 flex items-center justify-between gap-2">
                              <p className={`text-lg font-bold tabular-nums ${metric.key === 'net_monthly_cash_flow' ? (value >= 0 ? 'text-emerald-700 dark:text-emerald-200' : 'text-red-700 dark:text-red-300') : 'text-foreground dark:text-slate-100'}`}>
                                {metric.format(value)}
                              </p>
                              {isBest && (
                                <Badge variant="secondary" className="shrink-0 rounded-full border border-emerald-300/20 bg-emerald-400/10 text-xs text-emerald-700 dark:text-emerald-100">
                                  Best
                                </Badge>
                              )}
                            </div>
                ...
                >
                  {selectedClients.map((client) => {
                    ...
                    return (
                      <div 
                        key={client.id} 
                        className={`min-w-[220px] rounded-2xl border p-4 ${isBest ? 'border-emerald-300/30 bg-emerald-400/10 shadow-lg shadow-emerald-950/10' : 'border-border/60 dark:border-white/10 bg-muted/45 dark:bg-black/20'}`}
                      >
