...
import { industrialApi, useIndustrialCapex, type IndustrialCapexItem, type IndustrialCapexCategory } from '@/hooks/useIndustrialProperties';
import { toast } from '@/hooks/use-toast';

interface Props { propertyId: string; }

const CATEGORIES: { value: IndustrialCapexCategory; label: string }[] = [
  { value: 'roof', label: 'Roof' },
  { value: 'hardstand', label: 'Hardstand' },
  { value: 'racking', label: 'Racking' },
  { value: 'compliance', label: 'Compliance' },
  { value: 'sprinkler', label: 'Sprinkler' },
  { value: 'office_fitout', label: 'Office Fitout' },
...
export function IndustrialCapexTable({ propertyId }: Props) {
  const { items, loading, refresh } = useIndustrialCapex(propertyId);
  const [editing, setEditing] = useState<IndustrialCapexItem | null>(null);
  ...
  const handleDelete = async (id: string) => {
    ...
    const res = await industrialApi.deleteCapex(id);
              ...
              <TableBody>
                {items.sort((a, b) => a.year - b.year).map(i => (
                  <TableRow key={i.id}>
                    ...
                    <TableCell><Badge variant="secondary" className="capitalize">{i.category.replace('_', ' ')}</Badge></TableCell>
...
function CapexFormModal({ open, onClose, propertyId, item, onSaved }: {
  open: boolean; onClose: () => void; propertyId: string; item?: IndustrialCapexItem | null; onSaved: () => void;
}) {
  const isEdit = !!item;
  const [year, setYear] = useState(String(item?.year ?? new Date().getFullYear()));
  const [amount, setAmount] = useState(String(item?.amount ?? ''));
  const [category, setCategory] = useState<IndustrialCapexCategory>(item?.category ?? 'other');
  ...
  const handleSave = async () => {
    ...
    const res = isEdit
      ? await industrialApi.updateCapex(item!.id, payload)
      : await industrialApi.createCapex(payload);
    setSaving(false);
    if (res.error) {
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <Select value={category} onValueChange={(v) => setCategory(v as IndustrialCapexCategory)}>
