...
export function assessRiskOverlay(inputs: BorrowingInputs): RiskOverlayResult {
  ...
  if (inputs.dealProfile.assetCategory === 'commercial') {
    ...
    if (r.leaseDocumentationComplete === 'no') { score += 4; warnings.push('Lease documentation is incomplete.'); }
  } else {
    ...
    if (r.environmentalRisk === 'unknown') { score += 2; warnings.push('Industrial environmental status is unknown; lender may require environmental review.'); }
  ...
  const settings = {
    low: { noiHaircutPct: 0, lvrAdjustmentPct: 0, icrAdjustment: 0, dscrAdjustment: 0, additionalCapexReserve: 0 },
    medium: { noiHaircutPct: 0.05, lvrAdjustmentPct: -0.025, icrAdjustment: 0.05, dscrAdjustment: 0.05, additionalCapexReserve: inputs.dealProfile.assetCategory === 'industrial' ? 25000 : 0 },
    high: { noiHaircutPct: 0.12, lvrAdjustmentPct: -0.075, icrAdjustment: 0.15, dscrAdjustment: 0.1, additionalCapexReserve: inputs.dealProfile.assetCategory === 'industrial' ? 75000 : 0 },
    critical: { noiHaircutPct: 0.2, lvrAdjustmentPct: -0.15, icrAdjustment: 0.25, dscrAdjustment: 0.2, additionalCapexReserve: inputs.dealProfile.assetCategory === 'industrial' ? 125000 : 50000 },
  }[level];
  ...
  return { level, score, specialistReview: level === 'critical', warnings, riskAdjustedAssetCap: undefined, ...settings };
}
