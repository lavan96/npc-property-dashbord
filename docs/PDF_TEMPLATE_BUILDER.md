...
- **"Start from a reference"** (Import menu) — drag/click/paste a **PDF, image, or document**:
  ...
  heading styles + outline levels, formatted runs, hyperlinks, ordered/bullet lists via
  `numbering.xml`, paragraph alignment, tables, embedded images) and replicated onto the canvas through
...
Anchors are stored in the template schema on blocks/overlays as optional `anchors[]` metadata. They do not replace normal `{{binding.path}}` values; they explain which report-structure section/field owns a visual region and let the builder warn about required sections with no landing point, bindings outside the selected structure, duplicate anchors, and generated data that is not used in the design. Mapping a **field** anchor onto a selected text/image/table overlay also rewrites the overlay's value to the matching `{{sections.<id>.<field>}}` binding, so the section content flows into that region at render time. A section counts as **mapped** once its `body` field lands (`title`/`highlights` are optional decoration), and a required section whose field bindings already exist in the design (one auto-map click away) raises a warning rather than an activation blocker — only sections with no landing point at all hard-block activation. Each section row in the Cascade tab also previews the content it will ingest from the current sample data (or a loaded real report).
